...
def db_current_track(word, word_eol, userdata):
    is_db_running()
    track = subprocess.check_output(["deadbeef", "--nowplaying", "%t by %a [%b (%y)]"])
    xchat.command("me is listening to: {}".format(str(track, encoding='utf8')))

    return xchat.EAT_ALL
  
def db_current_track2(word, word_eol, userdata):
    is_db_running()
    track = subprocess.check_output(["deadbeef", "--nowplaying", "\"%a - %t\" | [%e/%l] | [%b (%y)]"])
    test = format(str(track, encoding='utf8'))
    if test == 'nothing' :
     return xchat.EAT_ALL
    
    tempstring = test.split('|', 2 )
    NowPlaying = "me is listening to: \00304" +tempstring[0] + "\003" + "\00308" +tempstring[1] + "\003\00303" + tempstring[2] + "\003"
    #print(NowPlaying)
    
    xchat.command(NowPlaying)
...
if __name__ == '__main__':
    ...
    xchat.hook_command('tnp',db_current_track2)
